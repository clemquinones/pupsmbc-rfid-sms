...
Public Class frmConnectionSetup
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If MB("Are you sure you want to  set  this  machine as " & IIf(rbLogIn.Checked, "log in", "log out") & "?", "?") = "n" Then Cursor = Cursors.Default : Exit Sub
        ...
        My.Settings.MACHINE = IIf(rbLogIn.Checked, "LOG IN", "LOG OUT")
        My.Settings.PCNAME = Environ("computername")
        My.Settings.Save()
        If recCount("tbleclient", "name='" & My.Settings.PCNAME & "'") > 0 Then
            UPDATEr("tbleclient", "machine='" & My.Settings.MACHINE & "',name='" & My.Settings.PCNAME & "',userid='',online=0", "name='" & My.Settings.PCNAME & "'")
        Else
            INSERTr("tbleclient", "'" & My.Settings.MACHINE & "','" & My.Settings.PCNAME & "','',0")
        End If
        Cursor = Cursors.Default
        lblSaved.Show() : btnSave.Text = "&Finish Settings" : LOGS("Connection configuration success.")
        gbConn.Enabled = False : rbLogOut.Enabled = False : rbLogIn.Enabled = False
    End Sub
