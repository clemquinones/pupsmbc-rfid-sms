...
#End Region
    ...
    Public Function saveImg(ByVal imgPath As String) As String   '@img
        'On Error GoTo c
        If imgPath = "" Then Return Nothing
        Dim FS As New IO.FileStream(imgPath, FileMode.Open, FileAccess.Read)
        Dim imgByte(FS.Length() - 1) As Byte
        FS.Read(imgByte, 0, imgByte.Length) : FS.Close()
        Dim MyParam As New MySqlParameter("@img", MySqlDbType.VarBinary, imgByte.Length, ParameterDirection.Input, False, 0, 0, Nothing, DataRowVersion.Current, imgByte)
        If CMD.Parameters.Contains("@img") Then CMD.Parameters.RemoveAt("@img")
...
c:      Return Nothing
    ...
    Public Function UserPass() As Boolean
    ...
    Public Function v(ByVal InputString_To_Validate_For_MySQL As String, Optional ByVal Replicate_Char_vchar As Boolean = False) As String
        Dim s As String = ""
        With InputString_To_Validate_For_MySQL
            s = .Replace("'", IIf(Replicate_Char_vchar = True, "''''", "\'"))
            s = .Replace("\", IIf(Replicate_Char_vchar = True, "\\\\", "\\"))
            s = .Replace("/", IIf(Replicate_Char_vchar = True, "////", "\/"))
            s = .Replace("(", IIf(Replicate_Char_vchar = True, "((((", "\("))
            s = .Replace(")", IIf(Replicate_Char_vchar, "))))", "\)"))
        End With
        Return s
    End Function
