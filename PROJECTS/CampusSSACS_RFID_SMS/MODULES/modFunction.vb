...
#End Region
    ...
    Public Function getRecipients(Optional ByVal where As String = "1") As String()
        Dim dRtemp As MySqlDataReader = DataReader("select s.number,p.number from tblestudent s,tbleparentinfo p where s.rfidtagid=p.rfidtagid and " & where)
    ...
    Public Function getResult(ByVal SQLCommand As String) As Object
        getResult = Nothing
        Dim dR As MySqlDataReader = DataReader(SQLCommand)
        If dR.Read Then
            getResult = dR.Item(0)
        End If
        dR.Close()
        ...
        If File.Exists(PathSearch) Then
            If CBool(getVal("tbleid", "value", "field like '%" & IIf(logUserType Like "Administrator", "SearchAdmin", "SearchCliet") & "%'")) = False Then
                Try : Shell(PathSearch & " /abovenormal")
                    UPDATEr("tbleid", "value=1", "field like '%" & IIf(logUserType.ToLower.ToString.Contains("administrator"), "SearchAdmin", "SearchCliet") & "%'")
                    Return True
                Catch ex As Exception : MB(ex.Message)
                End Try
            Else : Try : Shell("taskkill /im " & PathSearch & " /f")
                    UPDATEr("tbleid", "value=0", "field like '%" & IIf(logUserType.ToLower.ToString.Contains("administrator"), "SearchAdmin", "SearchCliet") & "%'")
                    RunSearch()
                Catch ex As Exception : MB(ex.Message)
                End Try
            End If
    ...
    Public Function AdminPass() As Boolean
        ...
        Return IIf(frmAdminPass.Tag = "OK", True, False)
    End Function
...
#Region "SOUND EFFECTS"
    Public Function Sounds(ByVal AUDIO As AUDIO) As Boolean
        If Not My.Settings.Sounds Then Exit Function
        Try
